...
pub(crate) fn list_items(input: String, code_theme: Theme, list_type: ListType) -> Vec<Node> {
    ...
    for line in lines {
        ...
        if segments.len() == 1 {
            let children = parse(segments[0].to_string(), code_theme.clone());
            ...
            let children = parse(input, code_theme.clone());
            tokens.push(Node::element("li", vec![], children));
        }
    }
